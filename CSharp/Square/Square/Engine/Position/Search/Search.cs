    ...
    public Eval Search(Draft wDraft, Eval mAlpha, Eval mBeta,
      ...
      try {
        ...
        if (goodMoves.Count > 0) {
          ...
          sb.mapMoves(Extensions.AppendPACN, goodMoves, State.Rule.IsChess960);
          sb.FlushLine();
        }
#endif
...
#if LazyMoveSort
        sb2.mapMoves(Extensions.AppendPACN, from sm2 in pm2 select sm2.Move, State.Rule.IsChess960);
#else
        sb2.mapMoves(Extensions.AppendPACN, from move2 in pm2 select move2, State.Rule.IsChess960);
#endif
        sb2.FlushLine();
#endif
        #endregion
        ...
        for (var nMoveIndex = 0; nMoveIndex < moves.Count; nMoveIndex++) {
          ...
          sb.AppendPACN(move, State.Rule.IsChess960);
          LogLine(sb.ToString());
#endif
